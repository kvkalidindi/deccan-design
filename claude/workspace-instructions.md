...
>
> **Templates are fetched, never remembered.** For an HTML or PDF document, fetch the canonical slot template at build time, every time — once per document build, including regenerations and revisions — and fill the copy that comes back:
...
> The unique query string is mandatory: the bare URL is served through caches that key on the full URL, and a plain fetch is routinely answered with a copy that predates the current release. After fetching, confirm the template's revision (header comment / generator meta) is not older than the skill bundle's version; if it is, refetch once with a new query string, then fall back to the bundled copy and say so. A copy bundled in the skill is a fallback for a session with no network, and the response must say when it was used. A template held in conversation context — including one this session fetched earlier — or lifted from a previous version of the document, is never acceptable.
>

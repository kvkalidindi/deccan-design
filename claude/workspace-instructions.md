...
> A cached template or skill bundle — from an earlier turn, an earlier document, or an installed copy — never overrides the fetched one. Only when the session cannot fetch (no network egress, fetch failure) fall back to the locally installed skill bundle, and say so in the response; never fall back silently.
>
> **Attribution.**
...
| Template fetch mechanics (unique query, per-build fetch, freshness floor, precedence) | `SKILL.md` § Fetching the template — hard rule |

...
> Fetch the slot template at build time, every time — once per document build, including regenerations — never reuse a copy held in the conversation (even one fetched earlier in it), bundled in a skill, or taken from an earlier version of the document. Append a unique cache-busting query string to every fetch (`?fetch=<current UTC timestamp>`): the bare URL is answered from caches and returns stale copies. Confirm the fetched revision (header comment / generator meta) is not older than the installed skill's version; refetch once with a new query string if it is. Before returning any HTML, confirm it carries the generator meta, `color-scheme: light only` and the pinned canvas rules; without them it renders dark-on-dark in the mobile previews. The canonical template:
>
